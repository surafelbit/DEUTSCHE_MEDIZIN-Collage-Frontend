...
const CrudSection = ({ title, data, setData }) => {
  ...
  const handleSubmit = () => {
    ...
    if (editingItem) {
      setData(
        data.map((d) =>
          d.modalityCode === editingItem.modalityCode ? { ...formData } : d
        )
      );
    } else {
  ...
  const handleDelete = (modalityCode) => {
    if (
      !window.confirm(
        `Are you sure you want to delete this modality? This action cannot be undone.`
      )
    )
      return;
  ...
  return (
    <div className="p-6 rounded-2xl shadow-xl transition-all duration-300 bg-white dark:bg-gray-800 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h2 className="text-2xl font-bold bg-blue-500 dark:white bg-clip-text text-transparent">
          {title}
        </h2>
      ...
      <input
        type="text"
        placeholder="Search Modalities by code or name"
        value={searchTerm}
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {paginatedData.map((item) => (
                ...
                >
                  <td className="p-4">{item.modalityCode}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="p-8 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-500 scale-95 animate-modal-in bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
            ...
            <input
              ...
              placeholder="Modality Code (e.g., MOD001)"
